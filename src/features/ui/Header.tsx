    ...
    <header className="fixed left-0 right-0 top-0 z-40 bg-[#FEF3E7] shadow-md">
      <div className="sm:grid flex grid-cols-[2.5fr_3fr_0.5fr] items-center text-lg justify-between sm:px-4 sm:py-1 px-2 py-3">
        ...
        <nav className="hidden  font-semibold text-sm md:flex items-center gap-10 ">
          <Link
            to="/homepage"
            className="hover:bg-dasadeep py-2 px-3 rounded-md"
          >
            Home
          </Link>

          <Link to="/about" className="hover:bg-dasadeep py-2 px-3 rounded-md">
            About
          </Link>
        ...
        <button onClick={() => dispatch(toggleNav())} className="md:hidden">
          <IoMenu className="size-10" />
        </button>
        <Link
          to="/login"
          className="bg-dasadeep hover:py-2 transition-all duration-200 border-2 border-transparent hover:border-zinc-700 hover:bg-transparent font-bold  text-sm  border sm:flex items-center hidden justify-center border-dasadeep py-1 px-1 rounded-md"
        >
          Login
        </Link>
      </div>
